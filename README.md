...
You may also find it helpful to use an app to manage your databases (highly suggested). One that I've found that works pretty well for our purposes is [Postico (Free)](https://eggerapps.at/postico/). The initial setup is simple, just replace my name in the `user` parameter to whatever `whoami` outputs in your terminal (probably will be your name).

![Postico Config](./Images/postico_config.png)

![Postico Main](./Images/postico_main.png)
...
5. Creating a `config/secrets/postgresql.json` file (add the basic json from the [PostgreSQL link](https://github.com/vapor-community/postgresql-provider))
